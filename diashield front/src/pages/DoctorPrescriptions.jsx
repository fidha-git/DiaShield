...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div className={`fixed top-6 right-4 sm:right-6 z-[100] flex items-center gap-3 px-5 py-3 rounded-xl border backdrop-blur-xl ${bg} shadow-2xl animate-slide-down max-w-[90vw]`}>
  ...
  return (
    <div className="space-y-6">
      <div className="page-container">
        ...
        {loading ? (
          <div className="bg-white dark:bg-[#0F172A]/90 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-lg shadow-slate-100/50 dark:shadow-none overflow-hidden">
            <table className="w-full"><tbody>{SkeletonRows()}</tbody></table>
          </div>
        ) : filtered.length === 0 ? (
          <div className="bg-white dark:bg-[#0F172A]/90 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-lg shadow-slate-100/50 dark:shadow-none flex flex-col items-center py-20 gap-4">
            <span className="material-symbols-outlined text-5xl text-slate-400 dark:text-slate-500">medication</span>
          ...
          <div className="bg-white dark:bg-[#0F172A]/90 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-lg shadow-slate-100/50 dark:shadow-none overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-900/50">
                    <th className="text-left p-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Patient</th>
                    <th className="text-left p-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Appointment Date</th>
                    <th className="text-left p-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Medicine</th>
                    <th className="text-left p-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Dosage</th>
                    <th className="text-left p-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Created</th>
                ...
                <tbody>
                  {filtered.map((presc) => {
                    const apt = appointmentsMap[presc.appointment_id];
                    return (
                      <tr key={presc.id} className="border-b border-slate-100 dark:border-slate-800/80 hover:bg-sky-50 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-sky-100 dark:bg-sky-500/20 flex items-center justify-center">
                              <span className="material-symbols-outlined text-sky-500 dark:text-sky-400 text-sm">person</span>
                            </div>
                            <span className="text-sm text-slate-900 dark:text-slate-100">{getPatientName(presc)}</span>
                          </div>
                        </td>
                        <td className="p-4 text-sm text-slate-900 dark:text-slate-200">
                          {apt ? formatDate(apt.date) : "—"}
                        </td>
                        <td className="p-4 text-sm text-slate-900 dark:text-slate-200">{presc.medicines}</td>
                        <td className="p-4 text-sm text-slate-900 dark:text-slate-200">{presc.dosage}</td>
                        <td className="p-4 text-sm text-slate-900 dark:text-slate-200">
...
function NewPrescriptionModal({ appointments, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white dark:bg-[#0F172A]/90 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-lg shadow-slate-100/50 dark:shadow-none p-6 w-full max-w-lg animate-scale-in max-h-[90vh] overflow-y-auto"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Appointment</label>
            <select value={selectedApt} onChange={(e) => setSelectedApt(Number(e.target.value))}
              className="w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/10 transition-all shadow-sm text-sm">
...
function ViewPrescriptionModal({ prescription, appointment, onClose }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white dark:bg-[#0F172A]/90 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-lg shadow-slate-100/50 dark:shadow-none p-6 w-full max-w-md animate-scale-in"
        onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            <span className="material-symbols-outlined text-sky-500">medication</span>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 pb-4 border-b border-slate-100 dark:border-slate-800/80">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-sky-500 to-cyan-600 flex items-center justify-center">
              <span className="material-symbols-outlined text-2xl text-white">prescriptions</span>
            </div>
            <div>
              <h4 className="text-sm font-bold text-slate-900 dark:text-slate-100">{appointment?.patient_name || `Appointment #${prescription.appointment_id}`}</h4>
...
function ConfirmDeleteModal({ prescription, patientName, onClose, onConfirm }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white dark:bg-[#0F172A]/90 border border-slate-100 dark:border-slate-800/80 rounded-2xl shadow-lg shadow-slate-100/50 dark:shadow-none p-6 w-full max-w-sm animate-scale-in"
        onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 text-center mb-6">
          Are you sure you want to delete the prescription for <span className="text-slate-900 dark:text-slate-100 font-semibold">{patientName}</span>?
        ...
        <div className="flex justify-center gap-3">
          ...
          <button onClick={onConfirm}
            className="px-5 py-2 rounded-lg bg-red-500/20 text-red-400 text-xs font-semibold hover:bg-red-500/30 transition-colors flex items-center gap-2">
...
function Field({ label, id, name, type, ...props }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id || name} className="text-xs font-semibold text-slate-500 dark:text-slate-400">{label}</label>
      {type === "textarea" ? (
